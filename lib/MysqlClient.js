...
class MysqlClient extends Client {
  ...
  _createConnection() {
    ...
    if (config.socketPath) {
      mysqlDriverConfig.socketPath = config.socketPath
    } else {
      mysqlDriverConfig.host = config.host
    }

    return new Promise((resolve, reject) => {
      const connection = dbDriver.createConnection(mysqlDriverConfig)
